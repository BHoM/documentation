...
This check will confirm the `cs` files changed within a pull request are compliant to [the documentation guidelines](Code-Compliance) for code files. This check will run only the compliance checks that have the Compliance Type of `documentation` in the table on the linked page.
